...
## 5.補遺 Phase 9: 対話的評価器生成と開放タスク N=4 への横展開
...
### 7.x Phase 9 開放タスク score_diff (Azure gpt-5.4, seed=42, N=8 サンプル)
...
| input_signature の parser が文言で揺れる | 同じ task_class でも goal 文の言い回しと LLM 能力で `inputs` の解釈が変わる (§7.5 で実証) | Phase 9b で **構造化対話 (Q1〜Q13 固定)** に切り替えて解消 |
| 開放タスクの評価軸が deterministic 2 種類のみ | char_limit + keyword_inclusion で 4 ドメイン試走 (§5 補遺)。spec_to_tests で +0.000 の天井効果 | Phase 10+ で LLM judge_panel/pairwise 軸を本実走に組み込む |
| Phase 9 のサンプル数は N=8 / seed=42 単発 | 統計的信頼性は別途 3-seed CI が必要 | Phase 10+ |
| Phase 9 試走は dialog_seed.yaml = Stage 3 までの replay | Stage 4-5 (サンプル判定 + judge 調整) は実装済だが本フェーズの試走では未利用 | Phase 10+ |
