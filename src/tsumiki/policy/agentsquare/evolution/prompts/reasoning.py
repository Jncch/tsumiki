"""tsumiki: AgentSquare evolution reasoning prompts (vendored from Apache-2.0).

Upstream: https://github.com/tsinghua-fib-lab/AgentSquare/blob/8f5b3fe5d8a32f9b59d20370823bef2a2c86928c/module_evolution/prompt_reasoning.py
Vendored at Phase 7e-3 (2026-06-19).
See docs/agentsquare_vendoring.md for vendoring policy.

Phase 7e-3 modifications:
- vendoring header docstring を追加. 本体 (archive エントリの長大な英語プロンプト + alfworld
  サンプルコード) は 上流のまま保持. entry は `get_init_archive_reasoning()`,
  `get_prompt_reasoning(archive)` の 2 関数のみ.
- archive 内 code 文字列の `llm_response(...)` は alfworld evolution の参照テンプレートと
  して保持 (実行されない). tsumiki 側の compose ラッパが evolution 出力を採用する際に
  `chat_fn` 経路へ書き換える想定 (Phase 7e-4).
- alfworld 固有テキスト (long prompt) は per-file-ignore で E501 許容.
"""
import json
...
def get_prompt_reasoning(current_archive, last_feedback=None):
    # Phase 7e-3: search/module_evolution.py 呼び出し側 (feedback あり) と互換性を取るため
    # last_feedback 引数を追加. 本実装では未使用 (上流の prompts は feedback 非対応).
    _ = last_feedback
